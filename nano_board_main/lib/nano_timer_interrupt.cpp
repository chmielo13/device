...
void (*_onTimerInterrupt)();



bool state = 0; //just for an example

const int INTERRUPT_CYCLE_LED_PIN = A2;
...
void TC5_Handler (void) {
    ...
    if(state == true) {
        // Temporarily disable usage of this led.
        // digitalWrite(INTERRUPT_CYCLE_LED_PIN,HIGH);
    } else {
        // Temporarily disable usage of this led.
        // digitalWrite(INTERRUPT_CYCLE_LED_PIN,LOW);
    }
    state = !state;
...
void timer_interrupt_setup(uint32_t sampleRate, void (*func)()) {
    
    pinMode(INTERRUPT_CYCLE_LED_PIN, OUTPUT);

    _onTimerInterrupt = func;
